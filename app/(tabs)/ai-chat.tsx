// app/(tabs)/ai-chat.tsx -- ПОЛНЫЙ КОД ФАЙЛА
...
import { useTasks } from '@/hooks/useTasks'; // <-- Важный импорт

const supabase = getSupabaseClient();

export default function AIChatScreen() {
  const { refreshTasks } = useTasks(); // <-- Получаем функцию для обновления задач
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Если ИИ что-то изменил, обновляем список задач в календаре
