// components/ui/TaskCreateModal.tsx -- ПОЛНЫЙ И ОКОНЧАТЕЛЬНЫЙ КОД
...
export function TaskCreateModal({ visible, onClose, task, initialDate }: TaskCreateModalProps) {
  ...
  const handleSave = async () => {
    if (!formData.title.trim()) { showAlert('Ошибка', 'Название задачи обязательно для заполнения'); return; }
    setLoading(true);
    try {
      ...
      else { showAlert('Успех', task ? 'Задача обновлена' : 'Задача создана'); onClose(); }
    } catch (error: any) {
      showAlert('Ошибка', error.message || 'Не удалось сохранить задачу');
    } finally {
      setLoading(false);
    }
  };
